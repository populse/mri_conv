...
public class ListBrukerSequence extends PrefParam implements ParamMRI2 {
	...
	public Object[] ListSeqBruker() throws IOException {
		 ...
		 * Format modification
		 *******************************************************/
		//resul[3] = new MRIFileManager.Dateformatmodif(resul[3].toString(), dictionaryMRISystem.get(listParamSeq[3]).get("format"), dictionaryJsonSystem.get(listParamSeq[3]).get("format")).getNewFormatDate();
		//resul[4] = new MRIFileManager.Dateformatmodif(resul[4].toString(), dictionaryMRISystem.get(listParamSeq[4]).get("format"), dictionaryJsonSystem.get(listParamSeq[4]).get("format")).getNewFormatDate();
		resul[6] = resul[6] + "D";
